...
struct dijkstra_t
{
    ...
    dijkstra_t() {}
    dijkstra_t(int f, int to, double v) : from(f), to(to), v(v) {}
...
void dijkstra(graph_t *g, int n, int s)
{
    ++mark_v;
    for(int i = 1; i <= n; ++i)
        dist[i] = 1.0e100;
...
//      std::printf("fa[%d] = %d\n", i, fa[i]);
    ...
    dijkstra_t *que = new dijkstra_t[n];
    for(int i = 1; i <= n; ++i)
    {
        root[i] = nil;
        que[i - 1].to = i;
    ...
    std::sort(que, que + n, std::greater<dijkstra_t>());
    for(int i = 1; i != n; ++i)
    {
        int u = que[i].to;
        node_t *r = nil;
